This is a post-receive hook for git to display commit messages to Campfire.

Use the `hookify` script to easily to install or upgrade the post-receive hooks on your repositories. Just run:

`./hookify /path/to/repository1 /path/to/repository2 ...`

h2. Revisions

*0.3 (March 27, 2009)*

* Add pre-speak and post-speak hooks, which can be used to call a batch of tests or whatever while keeping the post-receive hook untouched.
* Add hookify script to easily deploy the hook on several repositories

*0.2 (March 27, 2009)*

* Move Campfire configuration file to a separate .yml file

*0.1 (March 23, 2009)*

* Initial release